function[figure_handle, mean_values, std_values] = plot_overlap_vs_normal_firing_rate(time, p)
...
% for looking at firing rate of CA1 populaitions first
A       = CA1_A;
...
% decide what subplot to return depending on time of interest
if strcmp(time, 'first')
    subplotIdx = 1;
elseif strcmp(time, 'delay')
    subplotIdx = 2;
elseif strcmp(time, 'second')
    subplotIdx = 3;
end
...
for subpop = 1:3
    ...
    % log mean and std
    mean_values(subpop) = mean_value; 
    std_values(subpop) = sd_value; 
