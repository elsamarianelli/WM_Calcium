...
[i,j]       = find(C(:,pre_overlap_use)>0);                 %% E.M. think these have also been flipped
...
for t       = 1 : p.SimLength
    ...
    M.Isyn              = [zeros(p.in,1) ; sum((J.*M.x.*M.u.*M.delay_win(:,:,1)),1)']; % (4)  %% E.M. changed to ,1)' instead of ,2) because was in wrong dimensions
    ...
    pre                 = nonzeros(repmat(fired(1:p.in)',p.in,1) .* repmat((1:p.in),p.in,1) .* C);      %not sure whats being done here, need array size of repmapt to match C
