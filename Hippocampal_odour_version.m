...
overlap1 = mems{2};
ind1 = find((sum(C(overlap1, :))>2)); 
overlap2 = mems{1};
ind2 = find((sum(C(overlap2, :))>2)); 
...
% legend('10% of rest')
% 
% % working out mean spiking in overlapping cells vs non overlapping cells
% %during second odour 
...
%     M = simulate_dynapics_hipp(p, C, J, input, M, mems);
% 
%     overlapping = M.spikelog(p.in + coi, in.reactivation(1):in.reactivation(2));
%     non_overlapping = M.spikelog(p.in+full,  in.reactivation(1):in.reactivation(2));
% 
%     mean_overlapping = sum( overlapping, "all")/size(overlapping, 1);
%     mean_non_overlapping = sum(non_overlapping, "all")/size(non_overlapping, 1);
% 
%     overlapping_log(i) = mean_overlapping; non_overlapping_log(i) = mean_non_overlapping;
%     disp(i)
% end
% 
% plot(1:n_trials, overlapping_log); hold on; plot( 1:n_trials, non_overlapping_log);
