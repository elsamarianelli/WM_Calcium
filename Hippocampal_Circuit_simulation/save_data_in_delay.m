function[spikeCounts] = save_data_in_delay(C, J, p, ca3_ensembles, n_trials, variable, main_folder)
...
% need to change time3 and time4 back and get_train_data back to _db
    ...
    % time4 = time3+100; 
    time3 = input.simulation(2);
    time4 = input.reactivation(1); 
    
    % %%  Simulate hippocampal dynamics  over many trials labelling with reward/no reward
    [spikeCounts,~]	= get_train_data_2_odours(C, J, n_trials, p, ca3_ensembles, time3, time4);
    [spikeCounts_test, ~]	= get_train_data_2_odours(C, J, 50, p, ca3_ensembles, time3, time4);
