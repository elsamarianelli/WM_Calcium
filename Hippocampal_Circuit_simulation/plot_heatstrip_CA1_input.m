function[sortIdx] = plot_heatstrip_CA1_input(input_1, input_2, pair_spikes, C, ca3_ensembles)
...
sortIdx = [CA1_overlap_second, find(only_second_CA1), CA1_overlap_first, find(only_first_CA1)];
all_other = setdiff(1:200, sortIdx);
sortIdx = [sortIdx,all_other];

colour = [0.9*ones(1, length(CA1_overlap_second)), 0.7*ones(1, length(find(only_second_CA1))),...
...
set(gca, 'xtick', [20,70,140,200, 270], 'xticklabel', {'overlap/second','second only','overlap/first', 'first only' 'others'});
