function[fig_handle] = plot_performance_across_variable_change_pretrained_perceptron(variable_range, main_folder, variable_type)
...
% Load data
[data, ~] = loadData(fullFolderPath, main_folder);
...
for idx = 1:size(variable_range, 2)
    ...
    % Load data
    [~, data_test] = loadData(fullFolderPath, main_folder);
    disp(variable)
    % Repeat tests
    for j = 1:numTests
